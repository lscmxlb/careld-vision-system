...
/** 登录用户角色文案：医务人员按 staffRole 细分（医师/医生助理），其余按用户类型 */
export function userRoleText(userType?: number, staffRole?: number): string {
  if (userType === 6) {
    return STAFF_ROLE_MAP[staffRole ?? 0] || '医务人员'
  }
  return USER_TYPE_MAP[userType ?? 0] || '员工'
}
